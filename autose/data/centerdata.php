...
function schemeAdd($conn)
{
    ...
    //    // echo $sq;
    // die();
    $sql2 = "SELECT * FROM `tbl_servicescheme` WHERE `servicetype_id`='$stype' AND `model_id`='$model' AND `variant_id`='$variant'";
    $res = mysqli_query($conn, $sql2);
    if (mysqli_num_rows($res) > 0) {
        echo "<script> alert('Already exist');window.location ='../Addservicescheme.php';</script>";
    } else {
        $sql1 = "INSERT INTO `tbl_servicescheme`(`servicetype_id`, `licenceno`, `model_id`, `variant_id`,`department_id`,`km`, `amount`) VALUES ('$stype','$val','$model','$variant','$department','$km','$amount')";
        //die();
...
function searchAppointment($conn)
{
    $date = $_POST['date'];
    $licenceno = getSession('licenceno');
    $sql = "SELECT * FROM `tbl_appointment` WHERE appointment_status='0' AND licenceno='$licenceno' AND appointment_date='$date'";
    // print_r($sql);
    // return;
    $val = mysqli_query($conn, $sql);
    if ($val) {
...
function searchLeave($conn)
{
    $date = $_POST['date'];
    $licenceno = getSession('licenceno');
    $sql = "SELECT tbl_employee.first_name,tbl_employee.last_name,tbl_employee.department_id,tbl_leave.date,tbl_leave.reason,tbl_leave.leave_id FROM tbl_employee  JOIN tbl_leave ON tbl_employee.employee_id=tbl_leave.employee_id WHERE tbl_employee.licenceno='$licenceno' AND tbl_leave.status='1' AND tbl_leave.date='$date'";
    $val = mysqli_query($conn, $sql);
    if ($val) {
        while ($result = mysqli_fetch_array($val)) {
            ?>
