...
<div class="card-body">
    ...
    <form name="" id="login" method="post" action="data/centerdata.php" enctype="multipart/form-data" class="mt-5">
      ...
      <input type="text" hidden value="stockupdate" name="type">
      ...
      <div class="row">
       ...
       <?php
        ...
        // echo $val;
            ...
            $sql = "SELECT distinct tbl_model.model_name ,tbl_parts.model_id from tbl_parts inner JOIN tbl_model ON  tbl_parts.model_id=tbl_model.model_id WHERE tbl_model.brand_id='2'";
 ...
 ?>
       ...
       <select class=" form-control" name="fuel" id="fuel1" required>        
        ...
        <select class=" form-control" name="parts" id="parts" required>        
        <!-- echo '<option value="">Choose Parts</option>'; -->
        </select >
        </div>
        </div>
        <div class="col-md-6">
        <div class="md-form">
        <input type="number" class="form-control validate" name="quantity" id="quantity" placeholder="Quantity">  
        </div>
        </div>  
        </div>  
